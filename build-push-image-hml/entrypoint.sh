...
if [[ "$GITHUB_REF_NAME" == "staging" ]]; then
  REPOSITORY_URI_BRANCH="us-central1-docker.pkg.dev/bdsp-devtools/$REPOSITORY_NAME/staging"
  ...
  REPOSITORY_URI_BRANCH="us-central1-docker.pkg.dev/bdsp-devtools/$REPOSITORY_NAME/develop"
elif [[ "$GITHUB_REF_NAME" =~ ^release/ ]]; then
  REPOSITORY_URI_BRANCH="us-central1-docker.pkg.dev/bdsp-devtools/$REPOSITORY_NAME/release"
elif [[ "$GITHUB_REF_NAME" == "homolog" ]]; then
  REPOSITORY_URI_BRANCH="us-central1-docker.pkg.dev/bdsp-devtools/$REPOSITORY_NAME/homolog"
else
...
REPOSITORY_URI_PRD="us-central1-docker.pkg.dev/bdsp-devtools/$REPOSITORY_NAME/master"
...
if [[ "$GITHUB_REF_NAME" =~ ^release/ || "$GITHUB_REF_NAME" == "staging" || "$GITHUB_REF_NAME" == "homolog" || "$GITHUB_REF_NAME" == "master" || "$GITHUB_REF_NAME" == "main"  ]]; then
